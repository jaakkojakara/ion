...
@group(2) @binding(1)
var tex_not_used: texture_2d<f32>;
@group(2) @binding(2)
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let shadow_color = textureSample(tex, tex_sampler, in.tex_coords);
    ...
    let max_distance = 0.05; // 5% away from ground level
    let alpha = 1.0 - smoothstep(0.0, max_distance, height_distance);

    return vec4<f32>(0.0, 0.0, 0.0, shadow_color.a * alpha);
}